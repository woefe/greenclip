...
data Config = Config
  ...
  , staticHistoryPath          :: FilePath
  , imageCachePath             :: FilePath
...
appendToHistory sel history' = do
  ...
  where
    appendImage imgCtr extension bytes = do
      ...
      let imgPath = cachePth </> imgHash <.> extension
...
runDaemon = setHistoryFilePermission >> forever (go `catchAll` handleError)
  where
    ...
    purge path = do
      cachePth <- view (to imageCachePath)
      liftIO $ Dir.removeFile (cachePth </> path) `catchAll` const mempty
...
fromRofiStr :: FilePath -> Text -> IO Clip.Selection
fromRofiStr cachePth txt@(T.isPrefixOf "image/png " -> True)  = B.readFile (cachePth </> getHash txt <.> "png")  <&> Clip.Selection "greenclip" . Clip.PNG
fromRofiStr cachePth txt@(T.isPrefixOf "image/jpeg " -> True) = B.readFile (cachePth </> getHash txt <.> "jpeg") <&> Clip.Selection "greenclip" . Clip.JPEG
fromRofiStr cachePth txt@(T.isPrefixOf "image/bmp " -> True)  = B.readFile (cachePth </> getHash txt <.> "bmp")  <&> Clip.Selection "greenclip" . Clip.BITMAP
fromRofiStr _ txt = return $ Clip.Selection "greenclip" (Clip.UTF8 (T.map (\c -> if c == '\xA0' then '\n' else c) txt))

getHash :: Text -> FilePath
getHash = toS . fromMaybe mempty . lastMay . T.split (== ' ')
...
prepareDirs cfg = do
  ...
  let dirs = [ takeDirectory $ staticHistoryPath cfg
             , takeDirectory $ historyPath cfg
             , imageCachePath cfg
             , home </> ".config"
             , home </> ".cache"]
  mapM_ (Dir.createDirectoryIfMissing True) dirs
